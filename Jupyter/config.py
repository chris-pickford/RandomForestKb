...
class privateConfig(config):
    def __init__(self, OS, PROJECT_NAME):
        ...
        if OS == 'Local':
            ...
            self.ROOT = os.path.join(os.path.expanduser('SERVER DRIVE:'), os.path.sep, 'Chris Pickford', 'Projects')
            self.PROJECT_ROOT = os.path.join(self.ROOT, PROJECT_NAME)
            self.PACKAGE_ROOT = os.path.join(os.path.expanduser('SERVER DRIVE:'), os.path.sep, 'Datascience Packages')
